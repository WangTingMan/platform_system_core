 ...
 */
CUTILS_EXPORT Hashmap* hashmapCreate(size_t initialCapacity,
        int (*hash)(void* key), bool (*equals)(void* keyA, void* keyB));

/**
 * Frees the hash map. Does not free the keys or values themselves.
 */
CUTILS_EXPORT void hashmapFree(Hashmap* map);

/**
 * Hashes the memory pointed to by key with the given size. Useful for
 * implementing hash functions.
 */
CUTILS_EXPORT int hashmapHash(void* key, size_t keySize);
 ...
 */
CUTILS_EXPORT void* hashmapPut(Hashmap* map, void* key, void* value);

/**
 * Gets a value from the map. Returns NULL if no entry for the given key is
 * found or if the value itself is NULL.
 */
CUTILS_EXPORT void* hashmapGet(Hashmap* map, void* key);

/**
 * Removes an entry from the map. Returns the removed value or NULL if no
 * entry was present.
 */
CUTILS_EXPORT void* hashmapRemove(Hashmap* map, void* key);

/**
 * Invokes the given callback on each entry in the map. Stops iterating if
 * the callback returns false.
 */
CUTILS_EXPORT void hashmapForEach(Hashmap* map, bool (*callback)(void* key, void* value, void* context),
                    void* context);
 ...
 */
CUTILS_EXPORT void hashmapLock(Hashmap* map);

/**
 * Unlocks the hash map so other threads can access it.
 */
CUTILS_EXPORT void hashmapUnlock(Hashmap* map);
